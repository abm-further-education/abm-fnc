'use client';
import { inter, montserrat, tinos } from '@/app/layout';
...
function Menu() {
  ...
  return (
    <div className="my-40 md:my-60 px-16 md:px-0 flex flex-col justify-center items-center">
      ...
      <Divider />
      <p
        className={`${montserrat.className} text-sm md:text-base max-w-700 text-center text-white/80 mx-auto my-20`}
      >
      ...
      <p className={`${tinos.className} text-primary`}>Canapé Package</p>
      <div
        ...
        <div>
          ...
          <ul className="text-white mt-16">
            {menus.substantial.map((item, index) => {
              return (
                <li
                  className={`${inter.className} max-w-270 text-sm gap-5 flex`}
                  key={item}
                >
                  <span className="font-bold">{index + 1}.</span>
                  {item}
